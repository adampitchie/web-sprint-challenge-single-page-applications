...
It attemps to match a URL on loading. It does so without using a <Route>
1. How would you explain form validation to someone who has never programmed before?
Form validation attempts to check if forms are filled out in a way that is specified by the programmer. If not, it will show errors to get users to fill out the form the correct way.
1. In 1-2 sentences, define end to end testing.
End-to-end testing attempts to recreate an actual user moving around on the page. It is useful for detecting if things break when code changes.
